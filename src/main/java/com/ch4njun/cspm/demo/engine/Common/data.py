...
AWS_CURRENT_ID = sts_client.get_caller_identity()
...
class LowData:
    ...
    def load_vpc_low_data(self):
        if not self.vpcs:
            describe_vpcs = ec2_client.get_paginator('describe_vpcs').paginate()
            self.vpcs = [vpc for vpcs in describe_vpcs for vpc in vpcs['Vpcs']]
        if not self.security_groups:
            describe_security_groups = ec2_client.get_paginator('describe_security_groups').paginate()
        ...
        if not self.vpc_peering_connections:
            describe_vpc_peering_connections = ec2_client.get_paginator('describe_vpc_peering_connections').paginate()
        ...
        if not self.vpn_connections:
            describe_vpn_connections = ec2_client.describe_vpn_connections()
            self.vpn_connections = describe_vpn_connections['VpnConnections']
        if not self.nat_gateways:
            describe_nat_gateways = ec2_client.get_paginator('describe_nat_gateways').paginate()
    ...
    def load_iam_low_data(self):
        ...
        while not get_credential_report:
            try:
                get_credential_report = iam_client.get_credential_report()
        ...
        if not self.users:
            list_users = iam_client.get_paginator('list_users').paginate()
            self.users = [user for users in list_users for user in users['Users']]
        if not self.groups:
            list_groups = iam_client.get_paginator('list_groups').paginate()
            self.groups = [group for groups in list_groups for group in groups['Groups']]
        if not self.roles:
            list_roles = iam_client.get_paginator('list_roles').paginate()
            self.roles = [role for roles in list_roles for role in roles['Roles']]
        if not self.account_summary:
            self.account_summary = iam_client.get_account_summary()['SummaryMap']
        if not self.virtual_mfa_devices:
            list_virtual_mfa_devices = iam_client.get_paginator('list_virtual_mfa_devices').paginate()
        ...
        if not self.policies_only_attached:
            list_policies_only_attached = iam_client.get_paginator('list_policies').paginate(OnlyAttached=True)
        ...
        if not self.policies_local:
            list_policies_local = iam_client.get_paginator('list_policies').paginate(Scope='Local')
        ...
        if not self.account_password_policy:
            try:
                self.account_password_policy = iam_client.get_account_password_policy()['PasswordPolicy']
            except ClientError as e:
                self.account_password_policy = {}
        if not self.ssh_public_keys:
            for user in self.users:
                list_ssh_public_keys = iam_client.get_paginator('list_ssh_public_keys').paginate(UserName=user['UserName'])
        ...
        if not self.attached_user_policies:
            for user in self.users:
                list_attached_user_policies = iam_client.get_paginator('list_attached_user_policies').paginate(UserName=user['UserName'])
        ...
        if not self.access_keys:
            for user in self.users:
                list_access_keys = iam_client.get_paginator('list_access_keys').paginate(UserName=user['UserName'])
        ...
        if not self.user_policies:
            for user in self.users:
                list_user_policies = iam_client.get_paginator('list_user_policies').paginate(UserName=user['UserName'])
        ...
        if not self.group_policies:
            for group in self.groups:
                list_group_policies = iam_client.get_paginator('list_group_policies').paginate(GroupName=group['GroupName'])
        ...
        if not self.role_policies:
            for role in self.roles:
                list_role_policies = iam_client.get_paginator('list_role_policies').paginate(RoleName=role['RoleName'])
        ...
        if not self.server_certificates:
            list_server_certificates = iam_client.get_paginator('list_server_certificates').paginate()
            ...
            for name in server_certificates_name:
                self.server_certificates = iam_client.get_server_certificate(ServerCertificateName=name)['ServerCertificate']

    def load_cloudtrail_low_data(self):
        if not self.trails:
            describe_trails = cloudtrail_client.describe_trails()
            self.trails = describe_trails['trailList']
        if not self.trail_status:
            for trail in self.trails:
                self.trail_status[trail['TrailARN']] = cloudtrail_client.get_trail_status(Name=trail['TrailARN'])
        if not self.event_selectors:
            for trail in self.trails:
                self.event_selectors[trail['TrailARN']] = cloudtrail_client.get_event_selectors(TrailName=trail['TrailARN'])
    ...
    def load_cloudwatch_low_data(self):
        ...
        if not self.log_groups:
            describe_log_groups = logs_client.get_paginator('describe_log_groups').paginate()
        ...
        if not self.metric_filters:
            for log_group in self.log_groups:
                describe_metric_filters = logs_client.get_paginator('describe_metric_filters').paginate(logGroupName=log_group['logGroupName'])
        ...
        if not self.metric_alarms:
            describe_alarms = cloudwatch_client.get_paginator('describe_alarms').paginate()
    ...
    def load_s3_low_data(self):
        ...
        if not self.buckets_policy:
            for bucket in self.buckets:
                get_bucket_policy = s3_client.get_bucket_policy(Bucket=bucket['Name'])
                self.buckets_policy[bucket['Name']] = get_bucket_policy['Policy']
        if not self.buckets_acl:
            for bucket in self.buckets:
                get_bucket_acl = s3_client.get_bucket_acl(Bucket=bucket['Name'])
                self.buckets_acl[bucket['Name']] = get_bucket_acl['Grants']
        if not self.buckets_encryption:
            for bucket in self.buckets:
                try:
                    get_bucket_encryption = s3_client.get_bucket_encryption(Bucket=bucket['Name'])
        ...
        if not self.buckets_versioning:
            for bucket in self.buckets:
                get_bucket_versioning = s3_client.get_bucket_versioning(Bucket=bucket['Name'])
        ...
        if not self.buckets_lifecycle_configuration:
            for bucket in self.buckets:
                try:
                    get_bucket_lifecycle_configuration = s3_client.get_bucket_lifecycle_configuration(Bucket=bucket['Name'])
        ...
        if not self.buckets_object_lock_configuration:
            for bucket in self.buckets:
                try:
                    get_object_lock_configuration = s3_client.get_object_lock_configuration(Bucket=bucket['Name'])
        ...
        if not self.buckets_logging:
            for bucket in self.buckets:
                get_bucket_logging = s3_client.get_bucket_logging(Bucket=bucket['Name'])
    ...
    def load_rds_low_data(self):
        if not self.db_instances:
            describe_db_instances = rds_client.get_paginator('describe_db_instances').paginate()
        ...
        if not self.db_parameter_groups:
            describe_db_parameter_groups = rds_client.get_paginator('describe_db_parameter_groups').paginate()
        ...
        if not self.db_parameters:
            for db_parameter_group in self.db_parameter_groups:
                describe_db_parameters = rds_client.get_paginator('describe_db_parameters').paginate(DBParameterGroupName=db_parameter_group['DBParameterGroupName'])
        ...
        if not self.db_snapshots:
            describe_db_snapshots = rds_client.get_paginator('describe_db_snapshots').paginate()
        ...
        if not self.db_snapshot_attributes:
            for db_snapshot in self.db_snapshots:
                describe_db_snapshot_attributes = rds_client.describe_db_snapshot_attributes(DBSnapshotIdentifier=db_snapshot['DBSnapshotIdentifier'])
        ...
        if not self.event_subscriptions:
            describe_event_subscriptions = rds_client.get_paginator('describe_event_subscriptions').paginate()
        ...
        if not self.reserved_db_instances:
            describe_reserved_db_instances = rds_client.get_paginator('describe_reserved_db_instances').paginate()
        ...
        if not self.db_clusters:
            describe_db_clusters = rds_client.get_paginator('describe_db_clusters').paginate()
            self.db_clusters = [db_cluster for db_clusters in describe_db_clusters for db_cluster in db_clusters['DBClusters']]
        if not self.aliases:
            list_aliases = kms_client.get_paginator('list_aliases').paginate()
            ...
            if [backup_plan for backup_plan in self.backup_plans if 'Error' in backup_plan]:
                ...
                try:
                    for backup_plan in self.backup_plans:
                        self.list_backup_selections = backup_client.list_backup_selections(BackupPlanId=backup_plan['BackupPlanId'])
                        for backup_selection in self.list_backup_selections['BackupSelectionsList']:
                            get_backup_selection = backup_client.get_backup_selection(BackupPlanId=backup_plan['BackupPlanId'], SelectionId=backup_selection['SelectionId'])
    ...
    def load_ec2_low_data(self):
        ...
        if not self.instances:
            describe_instances = ec2_client.get_paginator('describe_instances').paginate()
        ...
        if not self.auto_scaling_instances:
            describe_auto_scaling_instances = autoscaling_client.get_paginator('describe_auto_scaling_instances').paginate()
        ...
        if not self.launch_configurations:
            describe_launch_configurations = autoscaling_client.get_paginator('describe_launch_configurations').paginate()
        ...
        if not self.launch_templates:
            describe_launch_templates = ec2_client.get_paginator('describe_launch_templates').paginate()
        ...
        if not self.launch_template_versions:
            for launch_template in self.launch_templates:
                describe_launch_template_versions = ec2_client.get_paginator('describe_launch_template_versions').paginate(LaunchTemplateName=launch_template['LaunchTemplateName'])
        ...
        if not self.route_tables:
            describe_route_tables = ec2_client.get_paginator('describe_route_tables').paginate()
        ...
        if not self.load_balancers_v1:
            describe_load_balancers = elb_client.get_paginator('describe_load_balancers').paginate()
        ...
        if not self.load_balancers_v2:
            describe_load_balancers = elbv2_client.get_paginator('describe_load_balancers').paginate()
        ...
        if not self.load_balancer_attribute_v1:
            for load_balancer in self.load_balancers_v1:
                describe_load_balancer_attributes = elb_client.describe_load_balancer_attributes(LoadBalancerName=load_balancer['LoadBalancerName'])
        ...
        if not self.load_balancer_attribute_v2:
            for load_balancer in self.load_balancers_v2:
                describe_load_balancer_attributes = elbv2_client.describe_load_balancer_attributes(LoadBalancerArn=load_balancer['LoadBalancerArn'])
    ...
    def load_ebs_low_data(self):
        if not self.snapshots:
            describe_snapshots = ec2_client.get_paginator('describe_snapshots').paginate(OwnerIds=[AWS_CURRENT_ID['Account']])
            self.snapshots = [snapshot for snapshots in describe_snapshots for snapshot in snapshots['Snapshots']]
        if not self.volumes:
            describe_volumes = ec2_client.get_paginator('describe_volumes').paginate()
            self.volumes = [volume for volumes in describe_volumes for volume in volumes['Volumes']]
        if not self.ebs_encryption_by_default:
            try:
                self.ebs_encryption_by_default = ec2_client.get_ebs_encryption_by_default()['EbsEncryptionByDefault']
            except ClientError as e:
    ...
    def load_cloudfront_low_data(self):
        if not self.distributions:
            list_distributions = cloudfront_client.get_paginator('list_distributions').paginate()
        ...
        if not self.distribution_detail:
            for distribution in self.distributions:
                get_distribution_config = cloudfront_client.get_distribution(Id=distribution['Id'])
    ...
    def load_kms_low_data(self):
        ...
        if not self.key_policies:
            for alias in self.aliases:
                get_key_policy = kms_client.get_key_policy(KeyId=alias['TargetKeyId'], PolicyName='default')
    ...
    def load_lambda_low_data(self):
        if not self.functions:
            list_functions = lambda_client.get_paginator('list_functions').paginate()
        ...
        if not self.function_policies:
            for function in self.functions:
                try:
                    get_policy = lambda_client.get_policy(FunctionName=function['FunctionName'])
